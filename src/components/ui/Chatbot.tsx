...
export default function Chatbot({
  ...
  const [messages, setMessages] = useState<{ text: string; isBot: boolean }[]>([
    {
      text: "Hi! I'm **Revil Assist**, here to help you with information about Revil events, workshops, and activities. What would you like to know?",
      isBot: true,
    },
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const model = genAI.current.getGenerativeModel({
        ...
        systemInstruction: `You are Revil Assist, an intelligent AI assistant for Revil.
...
Remember: Stay on topic! Only Revil-related queries get serious answers. Everything else gets a friendly redirect! 🎯`,
  ...
  return (
    <div
      className={`flex flex-col bg-black border border-primary/30 overflow-hidden ${is3D ? "w-full h-full" : "w-80 h-96 rounded-xl shadow-2xl"} ${className}`}
    >
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 scrollbar-hide">
        {messages.map((msg, idx) => (
          ...
          >
            <div
              className={`max-w-[85%] p-2 rounded-lg text-xs break-words overflow-wrap-anywhere ${
                msg.isBot
              ...
              {msg.isBot ? (
                <ReactMarkdown
                  components={{
                    ...
                    strong: ({ children }) => (
                      <strong className="font-bold text-primary break-words">
                        {children}
                      </strong>
                    ),
                    em: ({ children }) => (
                    ...
                    code: ({ children }) => (
                      <code className="bg-zinc-900 px-1 py-0.5 rounded text-primary break-all inline-block max-w-full">
                        {children}
                      </code>
                    ),
                  }}
      ...
      <form
        onSubmit={handleSend}
        className="p-2 border-t border-primary/20 bg-black/50"
      >
